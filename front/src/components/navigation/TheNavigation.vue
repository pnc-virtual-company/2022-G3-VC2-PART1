<template>
<nav  class="z-50  flex sticky -top-2 bg-blue-400 border-gray-200 px-2 sm:px-4 py-2.5 rounded dark:bg-gray-900">
  ...
  <div class="flex items-center md:order-2">
      <button type="button" class="flex mr-3 text-md bg-gray-800 rounded-full md:mr-0 focus:ring-4 focus:ring-gray-300 dark:focus:ring-gray-600" id="user-menu-button" aria-expanded="false" data-dropdown-toggle="user-dropdown" data-dropdown-placement="bottom">
        ...
        <img v-if="userData.gender== 'male'" class="w-7 h-7 rounded-full cursor-pointer" src="../../assets/Man.jpg" alt="user photo">
        <img v-else class="w-7 h-7 rounded-full cursor-pointer" src="../../assets/woman.png" alt="user photo">
      </button>
      <!-- Dropdown menu -->
      <div class="z-50 hidden my-4 text-base list-none bg-blue-400 rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600 block" id="user-dropdown"  style="position: absolute; inset: 0px auto auto 0px; margin: 0px; transform: translate(644px, 82px);">
        <div class="py-3 px-4">
          <span class="block text-lg font-bold">Name:   {{userData.first_name}} {{userData.last_name}}</span>
          <span class="block text-md font-medium truncate dark:text-gray-400">Email:  {{userData.email}}</span>
        </div>
        <ul class="py-1" aria-labelledby="user-menu-button">
          <li class="flex hover:bg-gray-100 items-center p-1">
            ...
            <router-link to="/user_detail" class="block py-2 px-4 text-md text-gray-700 dark:hover:bg-gray-600 dark:text-gray-200 dark:hover:text-white">Show Details</router-link>
          </li>
          <li class="flex hover:bg-gray-100 items-center p-1">
          ...
          <li data-modal-toggle="popup-modal" class="flex hover:bg-gray-100 items-center p-1">
    ...
    <div class="relative p-4 w-full max-w-md h-full md:h-auto">
        <div class="relative bg-gray-400 rounded-lg shadow dark:bg-gray-700">
            <button type="button" class="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-800 dark:hover:text-white" data-modal-toggle="popup-modal">
                <svg aria-hidden="true" class="w-5 text-white h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"></path></svg>
                <span class="sr-only">Close modal</span>
            </button>
            <div class="p-6 text-center">
                <svg aria-hidden="true" class="mx-auto mb-4 w-14 h-14 text-red-600 dark:text-gray-200" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                <h3 class="mb-5 text-lg font-normal text-black-500 dark:text-gray-400">Are you sure you want to logout from here?</h3>
    ...
    methods:{
      ...
      getUserData() {
        if(this.role == "teacherLogin"){
            axiosClient.get('/teacher/'+ this.userId)
            .then(response =>{
              this.userData = response.data
            })
        }else{
          axiosClient.get('/student/'+ this.userId).then((res)=>{
