<template>
  <div class="student">
    ...
    <student-leave v-if="selectOptions.length>=1"  :selectOptions='selectOptions'/>
      <div v-else-if="noData" class="flex justify-center" >
        <svg  class=" inline mr-2 w-10 h-10 text-gray-400 animate-spin dark:text-gray-600 fill-blue-600" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <div v-else class="flex justify-center -mt-9">
        <img class="w-96 h-96" src="../../assets/no_data.svg" alt="">
      </div>
</div>
</template>
<script>
...
export default {
  ...
  methods:{
    getListStudent(){
      axiosClient.get('get_student_request/'+this.student_id).then((req)=>{ 
    ...
    setTime(){
      setTimeout(() => {
        if(this.listStudentLeave.length == 0){
          this.noData = false;
        }
      }, 3000)
    }
